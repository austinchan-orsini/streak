    ...
    <div className="grid gap-3">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            {currentUser.avatarDataUrl ? (
              ...
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-lime text-[20px] font-[800] text-ink">
                {currentUser.username[0].toUpperCase()}
              </div>
            )}
            <div>
              <div className="text-[24px] font-[800] tracking-[-0.6px]">{currentUser.username}</div>
          ...
          <div className="flex flex-wrap gap-2">
            <span className="rounded-[18px] bg-lime px-3 py-2 text-[12px] font-[800]">
              DAY {Math.min(dayNum, 75)} OF 75
            </span>
            <button
              type="button"
              onClick={onLogout}
              className="rounded-[18px] border border-[rgba(63,51,38,0.10)] bg-white px-3 py-2 text-[12px] font-[700] transition hover:border-ink"
            >
              Log out
            </button>
          </div>
      ...
      <div className="grid gap-3 lg:grid-cols-4">
        {[
          ...
          <div key={item.title} className={`rounded-[18px] ${item.tone} p-3 text-ink`}>
            <div className="text-[12px] font-[700] uppercase tracking-[1.4px] opacity-90">{item.title}</div>
          </div>
        ))}
      </div>

      {/* Challenge rules + custom tasks / milestones */}
      <div className="grid gap-3 xl:grid-cols-[1.3fr_1fr]">
        <div className="space-y-3">
          <div className="soft-card p-4">
            <div className="text-[16px] font-[800] tracking-[-0.4px]">challenge rules</div>
            <div className="mt-2 space-y-1.5">
              {tasks.map((task) => (
                <div key={task.id} className="flex items-center gap-2.5 rounded-[14px] bg-panel px-3 py-2">
                  <div className="h-1.5 w-1.5 flex-none rounded-full bg-lime" />
          ...
          <div className="soft-card p-4">
            <div className="flex items-center justify-between gap-4 pb-3">
              <div>
                <div className="text-[16px] font-[800] tracking-[-0.4px]">custom tasks</div>
                <div className="text-[12px] text-[#8C7F6D]">Add or remove your personal habits.</div>
              </div>
              <button
                type="button"
                onClick={() => setShowAddForm((v) => !v)}
                className="rounded-[18px] bg-ink px-3 py-2 text-[12px] font-[700] text-bg"
              >
        ...
        <div className="soft-card p-5">
          <div className="text-[16px] font-[800] tracking-[-0.4px]">milestones</div>
          <div className="mt-3 space-y-2">
            {milestoneSteps.map((step) => {
              ...
              return (
                <div
                  key={step}
                  className="flex items-center gap-3 rounded-[14px] bg-panel px-4 py-3"
                >
